...
if(!$con)
 ...
 $Next = $_POST['Next'];
  ...
  $sql="INSERT INTO `order`(`Id`, `Name`, `Quantity`, `Price`, `Next`) VALUES 
  (NULL,'$Name','$Quantity','$Price','')";
...
if(!$con)
 ...
 $Name = $_POST['NameToDelete'];//get selected Name to delete
...
//============getOrderInformation
function getOrderInformation($Name)
{
//create connection
$con=mysqli_connect("localhost","fm2go","fm2go","fm2go");
if(!$con)
...
$sql = 'UPDATE `order` SET Name ="'.$Name.'", Quantity = "'.$Quantity.'", Price = "'.$Price.'" WHERE Name = "'.$Name.'"';
	echo $sql;
$qry = mysqli_query($con,$sql);//run query
return $qry;  //return query
}
